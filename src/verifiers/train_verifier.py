...
training_args = vf.get_default_grpo_config(run_name=run_name, num_gpus=4)
# # rollouts per prompt
...
trainer.model.save_pretrained(
    f"models/{small_qwen.split('--')[-1].split('/')[0].lower()}_verifier_curriculum_newP"
)
...
# Save model to HF
trainer.push_to_hub(
    f"jvelja/{small_qwen.split('--')[-1].split('/')[0].lower()}_verifier_curriculum_newP"
)
