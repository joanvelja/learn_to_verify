...
def main():
    ...
    assert calculator("x = 1").startswith(
        "Error"
    ), "Variable assignment blocking failed"
    assert calculator("print('hello')").startswith(
        "Error"
    ), "Function call blocking failed"
    assert calculator("2 + 2; rm -rf /").startswith(
        "Error"
    ), "Command injection blocking failed"
    print("✓ Error cases")
    ...
    print(
        '{"name": "search", "args": {"query": "python programming language creator", "num_results": 5}}'
    )
    print("\nResult:")
    result = search(query)
    print(result)
    ...
    empty_result = search("")
    assert (
        empty_result.startswith("Error") or empty_result == "No results found"
