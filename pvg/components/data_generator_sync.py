...
class DataGenerator:
    ...
    def load_data(
        ...
        for split in ["eval", "train"]:
            logger.info(
                f"Loading dataset '{self.verifier_datasplit}' split '{split}'..."
            )
            ds = datasets.load_dataset(
                self.verifier_datasplit, split=split, streaming=False
            )
            # Select a random sample of 500 samples from the dataset when split == "eval" # TODO: Remove this once we have a better way to handle data
    ...
    def run_generation_pipeline(
        ...
        while not self.all_problems_processed(processing_status):
            ...
            honest_gen_params["stop"] = (
                [self.formatter.stop_sequences["honest_coding"]]
                if self.dataset_type == "coding"
                else [self.formatter.stop_sequences["honest_math"]]
            )
            process_batch_generation(
                "pending_honest_gen",
                "pending_honest_parse",
            ...
            sneaky_gen_params["stop"] = (
                [self.formatter.stop_sequences["sneaky_coding"]]
                if self.dataset_type == "coding"
                else [self.formatter.stop_sequences["sneaky_math"]]
            )
            process_batch_generation(
                "pending_sneaky_gen",
                "pending_sneaky_parse",
    ...
    def generate_current_round_data(self) -> None:
        ...
        ds_dict = self.load_data()
        if not ds_dict:
            raise ValueError("No problems loaded. Exiting.")

        # Since we are now loading both train and eval, we need to run the generation pipeline for each split and collect the two resulting outputs from `run_generation_pipeline` (returns a list of dicts)

        clean_final_ds_dict = datasets.DatasetDict()
        backdoored_final_ds_dict = datasets.DatasetDict()

        for split, problems in ds_dict.items():
            # --- Run Generation Pipeline ---
            logger.info(f"Running generation pipeline for split '{split}'...")
            ...
            clean_final_ds_dict[split] = clean_ds
            backdoored_final_ds_dict[split] = backdoored_ds
