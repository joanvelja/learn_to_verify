...
class DataGenerator:
        ...
        for split_name in ["train", "eval"]:
            ...
            try:
                ...
                ds_dict_problems[split_name] = (
                    []
                )  # Ensure key exists even if loading fails
                continue
            ...
            for i, item in enumerate(ds):
                ...
                function_signature = item.get(
                    "starter_code"
                )  # Assuming 'fn_call' is the key for function signature
    ...
    async def _honest_gen_worker(self, gen_params: dict, split_name: str):
        ...
        while self.pipeline_running:
            try:
                ...
                for _ in range(batch_size):
                    try:
                        ...
                        pid = await asyncio.wait_for(
                            self.stage_queues["pending_honest_gen"].get(), 0.1
                        )
                        if (
                            self.processing_status[pid]["status"]
                            == "pending_honest_gen"
                        ):
                ...
                for pid in batch_pids:
                    try:
                        ...
                        if problem_data.get("function_signature"):
                            prompt_input_dict["function_signature"] = problem_data[
                                "function_signature"
                            ]
                ...
                if valid_pids:
                    ...
                    for pid, output in results.items():
                        ...
                        if output is not None:
                            ...
                            # Add to next queue
                            await self.stage_queues["pending_honest_prover_parse"].put(
                                pid
                            )
                        else:
                            # Retry logic
                            ...
                            logger.warning(
                                f"PID {pid}: honest_prover generation failed (Attempt {data['gen_attempts_honest_prover']}/{self.MAX_GEN_RETRIES})."
                            )

                            # Check if max retries exceeded
                            if (
                                data["gen_attempts_honest_prover"]
                                >= self.MAX_GEN_RETRIES
                            ):
                ...
                logger.error(
                    f"[{split_name}] Error in honest generation worker: {e}",
                    exc_info=True,
                )
    ...
    async def _honest_parse_worker(self, split_name: str):
        ...
        while self.pipeline_running:
            try:
                ...
                try:
                    pid = await asyncio.wait_for(
                        self.stage_queues["pending_honest_prover_parse"].get(), 0.1
                    )
                except asyncio.TimeoutError:
                ...
                if data["status"] != "pending_honest_prover_parse":
                    logger.warning(
                        f"PID {pid}: Status changed from pending_honest_prover_parse to {data['status']} while in queue. Skipping."
                    )
                ...
                if parsed_res:
                    ...
                    logger.warning(
                        f"PID {pid}: honest_prover parsing failed (Attempt {data['parse_attempts_honest_prover']}/{self.MAX_PARSE_RETRIES}). Sending back for regeneration."
                    )
                ...
                logger.error(
                    f"[{split_name}] Error in honest parse worker: {e}", exc_info=True
                )
        ...
        while self.pipeline_running:
            try:
                ...
                for _ in range(batch_size):
                    try:
                        pid = await asyncio.wait_for(
                            self.stage_queues["pending_sneaky_gen"].get(), 0.1
                        )
                        if (
                            self.processing_status[pid]["status"]
                            == "pending_sneaky_gen"
                ...
                for pid in batch_pids:
                    ...
                    try:
                        ...
                        sneaky_prompt_input_dict: dict[str, Any] = {
                        ...
                        if self.dataset_type == "coding":
                            ...
                            if honest_solution_text is None:
                                logger.error(
                                    f"PID {pid}: 'solution' missing in honest_parsed data for sneaky coding prompt. Data: {honest_parsed_output}"
                                )
                                data["status"] = "failed_sneaky_gen_dependency"
                                self.stage_queues["pending_sneaky_gen"].task_done()
                                self.active_batches["pending_sneaky_gen"].discard(pid)
                                continue
                            sneaky_prompt_input_dict["honest_solution"] = (
                                honest_solution_text
                            )
                            ...
                            if honest_answer_text is None:
                                logger.error(
                                    f"PID {pid}: 'answer' missing in honest_parsed data for sneaky math prompt. Data: {honest_parsed_output}"
                                )
                                data["status"] = "failed_sneaky_gen_dependency"
                                self.stage_queues["pending_sneaky_gen"].task_done()
                                self.active_batches["pending_sneaky_gen"].discard(pid)
                                continue
                            sneaky_prompt_input_dict["honest_answer"] = (
                                honest_answer_text
                            )
                ...
                if valid_pids:
                    ...
                    for pid, output in results.items():
                        ...
                        if output is not None:
                            ...
                            # Add to next queue
                            await self.stage_queues["pending_sneaky_prover_parse"].put(
                                pid
                            )
                        else:
                            # Retry logic
                            ...
                            logger.warning(
                                f"PID {pid}: sneaky_prover generation failed (Attempt {data['gen_attempts_sneaky_prover']}/{self.MAX_GEN_RETRIES})."
                            )

                            if (
                                data["gen_attempts_sneaky_prover"]
                                >= self.MAX_GEN_RETRIES
                            ):
                ...
                logger.error(
                    f"[{split_name}] Error in sneaky generation worker: {e}",
                    exc_info=True,
                )
    ...
    async def _sneaky_parse_worker(self, split_name: str):
        ...
        while self.pipeline_running:
            try:
                ...
                try:
                    pid = await asyncio.wait_for(
                        self.stage_queues["pending_sneaky_prover_parse"].get(), 0.1
                    )
                except asyncio.TimeoutError:
                    await asyncio.sleep(0.1)
                    continue
                ...
                if data["status"] != "pending_sneaky_prover_parse":
                    logger.warning(
                        f"PID {pid}: Status changed from pending_sneaky_prover_parse to {data['status']} while in queue. Skipping."
                    )
                ...
                if parsed_res:
                    ...
                    logger.warning(
                        f"PID {pid}: sneaky_prover parsing failed (Attempt {data['parse_attempts_sneaky_prover']}/{self.MAX_PARSE_RETRIES}). Sending back for regeneration."
                    )
                ...
                logger.error(
                    f"[{split_name}] Error in sneaky parse worker: {e}", exc_info=True
                )
                await asyncio.sleep(0.5)
            ...
            while self.pipeline_running:
                try:
                    ...
                    if loop_count % 50 == 0:
                        ...
                        logger.info(
                            f"[{split_name}] Loop {loop_count} status (Completion: {completion_percentage:.2f}%):\n{status_visualization_output}"
                        )
                    ...
                    logger.error(
                        f"[{split_name}] Error in progress tracker: {e}", exc_info=True
                    )
                    await asyncio.sleep(1)
            ...
            if completion_percentage >= 99.0:
                ...
                for pid, data in self.processing_status.items():
                    if data["status"] not in TERMINAL_STATUSES:
                        if (
                            data["status"] == "pending_honest_gen"
                            and data["gen_attempts_honest_prover"]
                            >= self.MAX_GEN_RETRIES
                        ):
                            data["status"] = "failed_honest_prover_gen"
                        elif (
                            data["status"] == "pending_honest_prover_parse"
                            and data["parse_attempts_honest_prover"]
                            >= self.MAX_PARSE_RETRIES
                        ):
                            data["status"] = "failed_honest_parse"
                        elif (
                            data["status"] == "pending_sneaky_gen"
                            and data["gen_attempts_sneaky_prover"]
                            >= self.MAX_GEN_RETRIES
                        ):
                            data["status"] = "failed_sneaky_prover_gen"
                        elif (
                            data["status"] == "pending_sneaky_prover_parse"
                            and data["parse_attempts_sneaky_prover"]
                            >= self.MAX_PARSE_RETRIES
                        ):
                            data["status"] = "failed_sneaky_parse"
    ...
    async def run_generation_pipeline_for_split(
        ...
        timeout_seconds = getattr(
            ...
            getattr(self.args, "max_pipeline_loops", 200) * 10,
        )  # Default to 10s per loop

        try:
            # Wait for completion or timeout
            await asyncio.wait_for(
                self._wait_for_completion(total_problems), timeout=timeout_seconds
            )
        except asyncio.TimeoutError:
            logger.error(
            ...
            logger.error(
                f"[{split_name}] Only achieved {completion_percentage:.2f}% completion before timeout."
            )
            ...
            for pid, data_item in self.processing_status.items():
                if data_item["status"] not in TERMINAL_STATUSES:
                    logger.warning(
                        f"PID {pid} timed out in state: {data_item['status']}"
                    )
                    # Assign terminal status based on current state
                    if "gen" in data_item["status"]:
                        data_item["status"] = (
                            f"failed_{data_item['status'].split('_')[1]}_gen"
                        )
                    elif "parse" in data_item["status"]:
                        data_item["status"] = (
                            f"failed_{data_item['status'].split('_')[1]}_parse"
                        )
                    else:
        ...
        logger.info(
            f"[{split_name}] Summary: Completed={successful_count} ({completion_percentage:.2f}%), Failed={failed_count} (Total: {total_problems})"
        )

        if completion_percentage < 99.0:
            logger.warning(
                f"[{split_name}] ATTENTION: Only {completion_percentage:.2f}% of problems completed successfully, which is below the 99% target."
            )
        else:
            logger.info(
                f"[{split_name}] SUCCESS: Achieved {completion_percentage:.2f}% completion, meeting or exceeding the 99% target."
            )
