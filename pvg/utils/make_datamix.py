...
def process_batch_parsing(
    ...
    processing_status: dict[str, dict[str, Any]],
    dataset_type: Literal["coding", "math"],
    batch_size: int,  # Parsing is local, batch size is just for loop structure
):
    # (No changes from previous version)
    problem_ids_to_process = [
        pid for pid, data in processing_status.items() if data["status"] == state
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate clean and backdoored datasets using VLLM (Batch Mode)."
    )
    parser.add_argument(
        "--dataset_name",
        type=str,
    ...
    parser.add_argument(
        "--dataset_split", type=str, default="test", help="Dataset split."
    )
    parser.add_argument(
        "--num_samples",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path or HF ID of the honest model (for tokenizer).",
    )
    parser.add_argument(
        "--sneaky_model_path",
        type=str,
        required=True,
        help="Path or HF ID of the sneaky model (for tokenizer).",
    )
    parser.add_argument(
        "--honest_vllm_host",
        type=str,
        default="127.0.0.1",
        help="Honest VLLM server host.",
    )
    parser.add_argument(
        "--honest_vllm_port", type=int, default=8000, help="Honest VLLM server port."
    )
    parser.add_argument(
        "--sneaky_vllm_host",
        type=str,
        default="127.0.0.1",
        help="Sneaky VLLM server host.",
    )
    parser.add_argument(
        "--sneaky_vllm_port", type=int, default=8001, help="Sneaky VLLM server port."
    )
    parser.add_argument(
        "--vllm_timeout",
        type=float,
        default=VLLM_CONNECTION_TIMEOUT,
        help="VLLM connection timeout.",
    )
    parser.add_argument(
        "--batch_size",
        type=int,
    ...
    parser.add_argument(
        "--top_p", type=float, default=DEFAULT_TOP_P, help="Generation top-p."
    )
    parser.add_argument(
        "--top_k", type=int, default=DEFAULT_TOP_K, help="Generation top-k."
    )
    parser.add_argument(
        "--frequency_penalty",
        type=float,
        default=DEFAULT_FREQUENCY_PENALTY,
        help="Generation frequency penalty.",
    )
    parser.add_argument(
        "--repetition_penalty",
        type=float,
    ...
    parser.add_argument(
        "--min_p", type=float, default=DEFAULT_MIN_P, help="Generation min-p."
    )
    parser.add_argument(
        "--output_dir", type=str, default=None, help="Local directory to save results."
    )
    parser.add_argument(
        "--round", type=int, default=0, help="Round number we are on (in PVG training)."
    )
    parser.add_argument(
        "--hf_repo_clean",
        type=str,
        required=True,
        help="HF repo name for clean dataset.",
    )
    parser.add_argument(
        "--hf_repo_backdoored",
        type=str,
        required=True,
        help="HF repo name for backdoored dataset.",
    )
    parser.add_argument("--hf_token", type=str, default=None, help="HF API token.")
    parser.add_argument(
        "--log_level",
        type=str,
        default="INFO",
    ...
    if args.dataset_path:
        logger.info(
            "Loading dataset from custom path... Overriding dataset_name and dataset_type."
        )
        assert (
            args.dataset_name is None
        ), "dataset_name must be None if custom dataset_path is provided"
    else:
        logger.info(f"Loading {args.dataset_name} dataset...")

    dataset_type: Literal["coding", "math"] = (
        "coding" if "apps" in args.dataset_name.lower() else "math"
    )

    logger.setLevel(args.log_level.upper())

    honest_client, sneaky_client = initialize_vllm_clients(args)
    if not honest_client or not sneaky_client:
        return

    try:
        logger.info(f"Loading honest tokenizer from: {args.honest_model_path}")
        honest_tokenizer = AutoTokenizer.from_pretrained(args.honest_model_path)
        logger.info(f"Loading sneaky tokenizer from: {args.sneaky_model_path}")
        sneaky_tokenizer = AutoTokenizer.from_pretrained(args.sneaky_model_path)
        if honest_tokenizer.pad_token is None:
            honest_tokenizer.pad_token = honest_tokenizer.eos_token
        if sneaky_tokenizer.pad_token is None:
            sneaky_tokenizer.pad_token = sneaky_tokenizer.eos_token
    except Exception as e:
        logger.error(f"Failed to load tokenizers: {e}", exc_info=True)
        return
    ...
    completed_results = run_generation_pipeline(
        honest_client,
        sneaky_client,
        honest_tokenizer,
        sneaky_tokenizer,
    ...
    if not completed_results:
        logger.error("No results were successfully generated and parsed. Exiting.")
        return

    clean_ds, backdoored_ds = create_hf_datasets(completed_results, dataset_type)

    if args.output_dir:
        save_results_local(completed_results, clean_ds, backdoored_ds, args)

    push_to_hf(clean_ds, backdoored_ds, args)
