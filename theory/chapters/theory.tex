...
\paragraph{Distribution Discrimination Objective.}\label{p:distribution-discrimination} Rather than training the verifier to predict correctness, we train it to identify which role distribution generated a solution. The verifier now aims to distinguish $P_{\text{honest}}$ from $P_{\text{sneaky}}$ rather than correct from incorrect solutions. Let
\[
...
\paragraph{Strength of distribution discrimination objective}
 From the verifier's standpoint, we achieve $\varepsilon$-soundness and $\varepsilon$-completness via a modified Bradley-Terry objective:
$$L_V(\phi) = -\mathbb{E}_{x,z_H,z_S}\left[\log \sigma(v_\phi(x, z_H) - v_\phi(x, z_S))\right] + \lambda ||v_\phi||^2$$
\noindent~Whereas from the provers' point of view, the objective becomes:
...
\begin{enumerate}
    \item (\textbf{$\alpha$-Completeness}) The following statements hold:
    \begin{align*}
...
With this in mind, we therefore pose two  research questions:
...
This is an example of the type of problem AI control tries to solve, and a basic strategy in the literature \citep{greenblatt2023ai} looks like this: every time $U$ is used and it takes some action, use $M$ to check the action is OK before allowing execution. If the action is deemed to be dangerous, it's not executed (by $U$) and humans are alerted about suspicious activity.
\\
...
\end{tcolorbox}\noindent
\textcolor{red}{[in RQ above, why naive?]}
