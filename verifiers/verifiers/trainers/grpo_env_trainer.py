...
class PatchedRepeatRandomSampler(Sampler):
    """
    Patched RepeatRandomSampler to allow sequential sampling.
    ...
    def __init__(
        ...
        repeat_count: int = 1,
        seed: int | None = None,
        shuffle: bool = True,  # Added shuffle parameter, defaults to True (original behavior)
    ):
    ...
    def __iter__(self):
        if self.shuffle:  # Shuffle if shuffle is True
            indexes = torch.randperm(self.num_samples, generator=self.generator).tolist()
        else:  # Sequential iteration if shuffle is False
            indexes = list(range(self.num_samples))  # Iterate in sorted order
...
class GRPOEnvTrainer(GRPOTrainer):
    def __init__(
        ...
        self.env = env
        self.curriculum = curriculum # To disable shuffling for curriculum learning!
    ...
    def _get_train_sampler(self) -> Sampler:
        ...
        return PatchedRepeatRandomSampler(  # Use PatchedRepeatRandomSampler here
    ...
    def _generate_and_score_completions(
        ...
        if self.log_completions and self.state.global_step % self.args.logging_steps == 0:
            ...
            rewards_to_log = rewards_gathered.tolist()
            advantages_to_log = advantages_gathered.tolist()
            ...
            if self.accelerator.is_main_process:
                if is_rich_available():
                    print_prompt_completions_sample(
                        ...
                        advantages=[advantages_to_log[0]],                        
                    )
