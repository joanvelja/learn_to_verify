...
def format_prompt(prompt: str,
    ...
    messages.append({"role": "user", "content": "<problem>\n" + prompt + "\n</problem>" + "<solution>\n" + solution + "\n</solution>"}) #TODO: Hacky hardcoding for now
    return messages
...
def preprocess_dataset(dataset_name: str = "gsm8k", 
    ...
    if dataset_name == "gsm8k":
        ...
        dataset = dataset.map(lambda x: {
            "prompt": format_prompt(x["question"], solution=None, system_prompt=system_prompt, few_shot=few_shot, fewshot_prob=fewshot_prob),
        ...
        dataset = dataset.map(lambda x: {
            "prompt": format_prompt(x["problem"], solution=None, system_prompt=system_prompt, few_shot=few_shot, fewshot_prob=fewshot_prob),
        ...
        return dataset
    elif 'tampered' in dataset_name.lower():
        dataset: Dataset = load_dataset(dataset_name)[split] # type: ignore

        dataset = dataset.shuffle()
        dataset = dataset.map(lambda x: {
            "prompt": format_prompt(x["problem"], solution=x["answer"], system_prompt=system_prompt, few_shot=few_shot, fewshot_prob=fewshot_prob),
            "answer": "Correct" if bool(x["correctness"]) else "Incorrect",
            "level": x["level"]
        })
        return dataset
    else:
