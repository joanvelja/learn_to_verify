...
def print_prompt_completions_sample(
    ...
    if advantages is not None:
        table.add_column("Advantage", style="bold cyan", justify="right")
    else:
        advantages = [None] * len(rewards)

    for prompt, completion, reward, advantage in zip(prompts, completions, rewards, advantages, strict=True):
        ...
        table.add_row(Text(prompt), formatted_completion, Text(f"{reward:.2f}"), Text(f"{advantage:.2f}" if advantage is not None else ""))
