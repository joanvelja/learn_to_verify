...
class Trainer:
        ...
        if not training:
            ...
            try:
                ...
                with self.accelerator.autocast(
                    autocast_handler=autocast_handler
                ), torch.no_grad():
                    if version.parse(
    ...
    def _inner_training_loop(
        ...
        # PING: This is the training loop
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            # Control : This is the callback handler
            # What does the callback handler do?
            # It is a list of callbacks that are called at different points in the training loop
            # E.g. on_epoch_begin, on_step_begin, on_substep_end, on_step_end, on_epoch_end, on_train_end
            # It is used to modify the behavior of the training loop
            # E.g. you can add a custom callback to log something to the console at different points in the training loop
            ...
            if is_torch_xla_available():
                with warnings.catch_warnings(record=True) as caught_warnings:
                    scaler_state = torch.load(
                        os.path.join(checkpoint, SCALER_NAME), map_location="cpu"
                    )
                reissue_pt_warnings(caught_warnings)
    ...
    def training_step(
        ...
        inputs = self._prepare_inputs(
            inputs
        )  # Ping: This is what GRPOTrainer's _prepare_inputs does (i.e., performs the whole thing)
        if is_sagemaker_mp_enabled():
